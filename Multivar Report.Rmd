...
```
The `NA` values are predominantly found within the popularity variables. Since the percentages are small (all less than 3%) and are seemingly random we will remove them for easier analysis.
...
The boxplots again show that the outliers seem to be heavily influencing the distributions thus we will consider omitting them.
...
The distributions now look much more normal so we will consider using a log-transformation within some of our analyses. 
...
The ``Comments`` residual plot appears to show some bias with a linear trend near the bottom of the plot. However, despite the plots not being perfectly homoscedastic, these residuals plots are certainly an improvement and thus a log-transformation will continue to be used for analysis.
...
The R-squared values are still low; however, they are greater than the R-squared values of the non-transformed model. The low values suggest that the model is not very good at making accurate predictions because there is a large amount of unexplained variance. 
